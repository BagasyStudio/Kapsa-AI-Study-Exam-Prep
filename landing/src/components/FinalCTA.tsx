"use client";

import { Button } from "@/components/ui/Button";
import { FloatingOrbs } from "@/components/ui/FloatingOrbs";
import { ScrollReveal } from "@/components/ui/ScrollReveal";
import { LINKS } from "@/lib/constants";

export function FinalCTA() {
  return (
    <section className="relative py-24 sm:py-32 overflow-hidden">
      <FloatingOrbs />
      <div className="bg-ethereal absolute inset-0" />

      <div className="relative mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 text-center">
        <ScrollReveal>
          <h2 className="font-heading text-3xl sm:text-4xl lg:text-5xl font-bold text-white tracking-tight">
            Ready to Ace Your
            <br />
            <span className="text-gradient">Next Exam?</span>
          </h2>

          <p className="mt-6 text-lg text-white/45 max-w-lg mx-auto">
            Download Kapsa free and start studying smarter today. No credit card
            required.
          </p>

          <div className="mt-8">
            <Button href={LINKS.appStore} size="lg" className="animate-pulse-glow">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z" />
              </svg>
              Download on the App Store
            </Button>
          </div>

          <p className="mt-4 text-sm text-white/25">
            Available on iOS · Free to download
          </p>
        </ScrollReveal>
      </div>
    </section>
  );
}
